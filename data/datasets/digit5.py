...
class Digit5(DatasetBase):
    ...
    def __init__(self, cfg):
        ...
        train_s = self._read_data(cfg.DATASET.SOURCE_DOMAINS, split='train')
        test_s = self._read_data(cfg.DATASET.SOURCE_DOMAINS, split='test')
        train_t = self._read_data(cfg.DATASET.TARGET_DOMAINS, split='train')
    ...
    def _read_data(self, input_domains, split='train'):
        ...
        for domain, dname in enumerate(input_domains):
            ...
            for impath, label in items_d:
                item = Datum(impath=impath, label=label, domain=domain, classname=str(label), domainname=dname, id=id)
                items.append(item)
                id = id + 1
